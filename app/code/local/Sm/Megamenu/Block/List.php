...
// 		$activedClassName = ($this->isActived($item) AND $this->hasActivedType($item))?$prefix.'actived':'';
		...
		$html = '<div class="'.$divClassName.' '.$firstClassName.' '.$prefix.'id'.$item->getId().' '.$align_right.' '.$activedClassName.' '.$item->getCustomClass().'class">';
		...
		$icon_title = ($this->hasIcon($item))?'<span class="icon-v"><img alt="icon item" src='.$this->filterImage($item).' /></span><span style="" class="'.$prefix.'icon">'.$title.'</span>':$title;		
