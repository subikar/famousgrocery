...
}), function($)
{
	...
	function($)
	{
		$.fn.filmore = function(opts, callback)
		{
			...
			{
				$("iframe[src]", opts.slide_id).length ? $("iframe[src]",  opts.slide_id).each(function()
				{
					var a = $(this),
						i = a.attr("src");
					a.attr("data-src", i);
			...
			//var array_enfinity_content_width = enfinity_content_width.split(' ');
			//enfinity_content_width = parseInt(array_enfinity_content_width[1]);
			...
			function resizeFont() {
				...
				$(".filmore_caption", $(opts.slide_id)).each(function(){
					var fsize = $(this).attr('data-fontsize'), _fsize = $(this).css("font-size"),  _fsize = _fsize.search('px') !== -1 ? _fsize.replace('px','') : '';
			...
			{
				f=e+1;
				$(opts.pagination).append('<a href="#" class="filmore_pag filmore_pag_' + e + '" data-pag="' + e + '">' + f +  "</a>")
			});
