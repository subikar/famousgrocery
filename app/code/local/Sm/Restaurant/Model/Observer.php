...
-------------------------------------------------------------------------*/
//die('aaa');
class Sm_Restaurant_Model_Observer {

	public function less_compile( $observer ){	
	
		$setting = Mage::helper('restaurant/data');
		$less_theme_compile     = Mage::getStoreConfig('restaurant_cfg/advanced/less_compile');
		$preset_name 			= Mage::getStoreConfig('restaurant_cfg/general/theme_color');
		$device_responsive      = Mage::getStoreConfig('restaurant_cfg/theme_layout/device_responsive'); 
		$direction      		= Mage::getStoreConfig('restaurant_cfg/theme_layout/direction');
			...
			if ( class_exists('Less_Parser') && $less_theme_compile ){
				...
				if ( file_exists(LESS_PATH.'/theme.less') && $less_theme_compile  ){
					...
					if( $device_responsive == 1 ){
						...
						$less->parseFile(LESS_PATH.'/path/yt-responsive-header.less', $skin_base_url.'css/');
