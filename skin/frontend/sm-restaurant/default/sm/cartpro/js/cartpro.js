...
var ajaxCartPro = {
	initialize: function() {
		...
		this.cus_wishlist = $$('.my-wishlist .count-item')[0]; /* Enter class name of My Wishlist */
		this.header_compare = $$('.link-compare .count-item')[0]; /* Enter class name of Header Compare */
			...
			if (res) {
				if (res.success) {
					...
					if (_this.ckClsExist(_this.cpmodal_compare) && res.is_compare) {
						...
						if (_this.header_compare && _this.ckClsExist(_this.header_compare)) _this.header_compare.update(res.nb_items);
					...
					if (_this.ckClsExist(_this.cpmodal_wishlist) && res.is_wishlist) {
						...
						if (res.nb_items && _this.ckClsExist(_this.cus_wishlist)) _this.cus_wishlist.update(res.nb_items);
						_this.eventForWishlist();
					}
	...
	popWinCompare: function () {
		window.removeItem = function(url) {
			if (typeof window.opener !== 'undefined' && window.opener !== null) {
				...
				window.ajaxCartPro.ajaxCartSubmit(url, 'compare');
				window.location.reload();
			}
		}
	}
...
var setLocation = (function() {
	return function(url) {
		if (typeof enable_ajax_cart !== 'undefined' &&  enable_ajax_cart && url.search('checkout/cart/add') != -1)
		...
		else if (typeof enable_ajax_compare !== 'undefined' &&  enable_ajax_compare && url.search('catalog/product_compare') != -1 && url.search('catalog/product_compare/index/items') === -1)
			...
			if (typeof enable_ajax_cart !== 'undefined' &&  enable_ajax_cart && typeof cartpro_baseurl !== 'undefined'  && url !== cartpro_baseurl && url.search('blog') == -1 && url.search('limit') == -1 && url.search('onepage') == -1 && url.search('#tab-product-view3') == -1 && url.search('catalog/product_compare/index/items') === -1) {
